...
int perform(object me, object target)
{
	...
	&& !me->query("guild") )
        return notify_fail("你不是蜀山剑派弟子，不会用「飞龙探云手」?\n");
	...
	&& me->query_skill("hunyuan-zhang",1) < 50 )
		return notify_fail("你的混元掌修为太浅，不会使用「飞龙探云手」。\n");
	...
	if(random(me->query_skill("dodge",1)+me->query_skill("unarmed",1))>target->query_skill("dodge",1))
	{
		...
		if(!sizeof(inv))	{
			...
			thing->move(me);
			msg +=HIM"$N使出飞龙探云手的绝招, $n顿时觉得眼前一花，$N妙手空空，从$n身上得到"+thing->query("unit")+thing->query("name")+"！\n"NOR;
		}
	} else 
	{
		me->start_busy(1);
