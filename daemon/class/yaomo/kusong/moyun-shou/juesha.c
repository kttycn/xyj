...
{       
	...
	&& !me->query("guild") )
           return notify_fail("“摩云绝杀”是火云洞的不传之密！\n");
	if( (int)me->query_skill("moyun-shou", 1) < 100 ||
	(int)me->query_skill("unarmed", 1) < 100 )
		return notify_fail("你有的功夫还不够娴熟，不会使用「摩云绝杀」。\n");
/*
...
*/
	...
	me->query_skill("moyun-shou", 1) < 100)
		return notify_fail("以你现在的空手修为还使不出「摩云绝杀」。\n");
