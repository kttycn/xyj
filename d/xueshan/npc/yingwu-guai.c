...
int rank_me()
{
	...
	if( me->query("title") == "ÕòÉ½·ï»ËÃ÷Íõ" 
	|| me->query("title") == "ÎŞ·¨ÎŞÌì´óĞ°Ê¥" )
	...
	if( (string)me->query("family/family_name") == "´óÑ©É½"){
		if( exp <= 500000 ) {
			...
			me->set("nick", HIW"Î÷ÓòÑ©É½"NOR+RED"¡ô"BLU"Ñ²É½Ğ¡Ä§"NOR);
		} else if(exp <= 2000000 ){
			me->set("nick", HIW"Î÷ÓòÑ©É½"NOR+RED"¡ô"YEL"±ù¹ÈÑ²Ê¹"NOR);
		} else if(exp <= 5000000 ){
			me->set("nick", HIW"Î÷ÓòÑ©É½"NOR+RED"¡ô"RED"»¤ÅÉ×ğÕß"NOR);
		} else if(exp <= 10000000 ){
			me->set("nick", HIW"Î÷ÓòÑ©É½"NOR+RED"¡ô"MAG"´óÅô½«¾ü"NOR);
		} else {
			me->set("nick", HIW"Î÷ÓòÑ©É½"NOR+RED"¡ô"CYN"¾ÅÒõº®Ä§"NOR);
		}

		tell_object(me, "ğĞğÄ½«¾ü¶ÔÄãËµ£ºÄã¿ÉÒÔ×ö"+me->query("nick")+"¡£\n");
		tell_room(environment(me),
"ğĞğÄ½«¾ü¶Ô"+me->query("name")+"Ëµ£ºÄã¿ÉÒÔ×ö"+me->query("nick")+"¡£\n", ({me, me}));
