...
void create()
{
	set("short", HIY"地下三号岩洞"NOR);
	set("long", @LONG
这里就是凤凰星的地下三号岩洞，这里经常有
很多怪兽出没，不过据闻怪兽身上可有好多好
东东。(dig yandong)
LONG
	);
	set("exits", ([
		"down"  : __DIR__"yandong4",
		"up"     : __DIR__"yandong2",
	]));
	setup();
}
...
int do_dig(string arg)
{
	...
	if (me->query_temp("digdong3")!=1) return notify_fail("你还是先去领任务吧！\n");
	if ( me->query("kee") < 10 ) return notify_fail("你的气血不足，休息一下再来吧！\n");
	if ( me->query("combat_exp") > 500000)
		return notify_fail("你已经不需要通过杀怪兽来提高自己的战斗力了！\n");
