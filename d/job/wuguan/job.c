...
int accept_tool(object npc,object ob,object obj)
{
	if(!ob->query_temp("wuguan/tool"))
	{
		message_vision("$n对$N笑道：怎么？贿赂我？我可不会被你腐蚀的。\n",ob,npc);
		return 0;
	}
	if(interactive(obj))
	...
	message_vision("$n拍拍$N的头，嘻嘻笑道：好样的。\n",ob,npc);
...
int real_give_gifts(object ob)
{
	...
	ob->add("daoxing",(dx=(int)job["bonus"]+random(job["exp"])));
	...
	tell_object(ob,HIY"恭喜你获得"+COMBAT_D->chinese_daoxing(dx)+"道行、"+
...
int job_saodayuan(object ob,string arg)
{
	...
	if(ob->query_temp("wuguan/saodayuan")<20)
	{
		...
		if ( (int)ob->query_skill("staff", 1) < 20 && random(10)>5 ){  
			write(HIM"你在扫院中对于杖的用法有些体会!\n"NOR);
			ob->improve_skill("staff", (int)(ob->query("str") / 10));
		}
	}
	if(random(100) > 80)
	{
...
int job_diaoyu(object ob,string arg)
{
	...
	if ( (int)ob->query_skill("halberd", 1) < 20 && random(10)>5 ){  
		write(HIM"你在钓鱼中对于戟法的用法有些体会!\n"NOR);
		ob->improve_skill("halberd", (int)(ob->query("str") / 10));
	}
	ob->start_busy(1);
...
int end_fishing(object ob)
{
	...
	if ( (int)ob->query_skill("spells", 1) < 20 && random(10)>5 ){  
		write(HIM"你在钓鱼中对于法术的修习有些体会!\n"NOR);
		ob->improve_skill("spells", (int)(ob->query("spi") / 10));
	}
	return 1;
}
...
int give_fish(object npc,object ob,object obj)
{
	...
	message_vision(CYN"$n笑嘻嘻地对$N说：你如此精明能干，他日定有所成。\n"NOR,ob,npc);
	return give_gifts(ob);
}
...
int job_dashui(object ob,string arg)
{
	...
	if ( (int)ob->query_skill("parry", 1) < 20 && random(10)>5 ){  
		write(HIM"你在打水中对于拆招卸力之法的用法有些体会!\n"NOR);
		ob->improve_skill("parry", (int)(ob->query("str") / 10));
	}
	ob->receive_damage("kee",10);
	ob->start_busy(3);
	return 1;
...
int job_jiaoshui(object ob,string arg)
{
	...
	if(ob->query_temp("wuguan/jiaoshui")>20)
	{
		...
		if ( (int)ob->query_skill("hammer", 1) < 20 && random(10)>5 ){  
			write(HIM"你在浇水的过程中对于锤法的用法有些体会!\n"NOR);
			ob->improve_skill("hammer", (int)(ob->query("str") / 10));
		}
...
int job_chucao(object ob,string arg)
{
	...
	if(ob->query_temp("wuguan/chucao")>20)
	{
		...
		if ( (int)ob->query_skill("spear", 1) < 20 && random(10)>5 ){  
			write(HIM"你在锄草的过程中对于枪法的用法有些体会!\n"NOR);
			ob->improve_skill("spear", (int)(ob->query("str") / 10));
		}
	...
	ob->receive_damage("kee",3);
	return notify_fail("你轮起锄头，一下一下地锄了起来。\n");
}
...
int job_pichai(object ob,string arg)
{
	...
	if(random(100) < 20)
	{
		...
		ob->receive_damage("kee",10);
		ob->receive_damage("sen",10);
		if ( (int)ob->query_skill("blade", 1) < 20 && random(10)>5 ){  
			write(HIM"你在劈柴中对于刀法的用法有些体会!\n"NOR);
			ob->improve_skill("blade", (int)(ob->query("str") / 10));
		}
		return 1;
	}
...
int job_shenghuo(object ob,string arg)
{
	...
	message_vision("$N拿出一根柴火在一旁引着，放到炉子里。\n",ob);
    return 1;
}
...
int over_sheng(object ob)
{
	...
	{
	 	ob->receive_damage("kee",5);
	 	message_vision("$N费了半天劲，脸被烟熏的像个大花猫，但却没把火生着。\n",ob);
		if ( (int)ob->query_skill("force", 1) < 20 && random(10)>5 ){  
			write(HIM"你在生火中对于内功的修炼有些体会!\n"NOR);
			ob->improve_skill("force", (int)(ob->query("con") / 10));
		}
        return 1;
	}
	
	message_vision("但见"+HIR"火苗"NOR+"越生越旺，$N拿起吹火棍加紧吹了几下，终于把火生着了。\n",ob);
	ob->receive_damage("kee",10);
	return give_gifts(ob);
}
...
int job_zhengli(object ob,string arg)
{
	...
	if(ob->query_temp("wuguan/zhenglishujia")<15)
	{
		...
		ob->start_busy(1);
		ob->receive_damage("kee",5);
		if ( (int)ob->query_skill("literate", 1) < 20 && random(10)>5 ){  
			write(HIM"你在整理书架中对于读书写字的用法有些体会!\n"NOR);
			ob->improve_skill("literate", (int)(ob->query("int") / 10));
		}
...
int job_cashujia(object ob,string arg)
{
	...
	if(ob->query_temp("wuguan/cashujia")<10)
	{
		...
		if ( (int)ob->query_skill("unarmed", 1) < 20 && random(10)>5 ){  
			write(HIM"你在擦书架中对于扑击格斗之技的用法有些体会!\n"NOR);
			ob->improve_skill("unarmed", (int)(ob->query("str") / 10));
		}
	}
