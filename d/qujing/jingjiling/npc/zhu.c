...
int accept_fight(object ob)
{
	...
	if(me->is_fighting() )
		return notify_fail("拂云叟对你说：没见老夫正忙着那么？\n");
	if( (int)me->query("kee")*100/(int)me->query("max_kee") < 30 )
		return notify_fail("拂云叟对你说：老夫体乏无力，比不得，比不得。\n");
	if ( who->query_temp("obstacle/jingjiling/fuyun sou") )
		return notify_fail("拂云叟对你说：老夫甘拜下风，不比了，不比了。\n");
	if( who->query_temp("poem_zhu") ) {
		fight_name = me->query("name")+"("+capitalize(me->query("id"))+")";
		command("say 好，好，你我切搓武艺，点到为止！\n");
		who->set_temp("jingji_fight",fight_name);
		reset_me ();
		call_out ("check_fight",1,who);
		return 1;
	}
	command("say 且待老夫考你一考。\n");
	who->set_temp("answer_zhu", 1);
...
{	
	object me=this_object();
	message_vision("$N对$n问道：“五岳寻仙不辞远”的下句是什么？\n", me, who);
...
int do_answer(string arg)
{
	...
	if( this_player()->query_temp("answer_zhu") ) {

		if( strsrch(arg, "一生好入名山游") >=0  ) {
			command("haha");
			command("say 不错，不错！老夫就与你过两招。\n");
			this_player()->set_temp("poem_zhu", 1);
			this_player()->delete_temp("answer_zhu");
			this_object()->delete_temp("check_answer");
