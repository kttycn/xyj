...
int test_player(object who)
{
  ...
  if (who->query("obstacle/nverguo") == "stomachache")
  {
    reject_player (msg+"怀了一水胎，居然挺着大肚子来见我！\n",who);
    return 1;
  }  

  if (who->query("obstacle/nverguo") == "marriage")
  {
  ...
  if (who->query("obstacle/nverguo") == "company")
  {
...
void send_to_greenyard (object who)
{
  ...
  if (who->query("gender") != "女性")
    who->set("obstacle/nverguo", "marriage");
  else
    who->set("obstacle/nverguo", "company");
  who->save();
  call_out ("invite_player",1,who);
} 
