...
void create()
{
  ...
  setup();
  carry_object("/d/qujing/nverguo/obj/skirt")->wear();
...
void classified_greeting(object me)
{
  ...
  if (me->query("obstacle/nverguo") == "done")
    return;

  if (me->query("obstacle/nverguo") == "marriage")
  {
    message_vision (name+"对$N说：大家都在议论你的婚事哩，快去见公主。\n",me);
      return;
  }  

  if (me->query("obstacle/nverguo") == "company")
  {
    message_vision (name+"认出了$N，说道：哟，公主还在等你作伴呢。\n",me);
      return;
  }  

  if (me->query("obstacle/nverguo") == "stomachache")
  {
