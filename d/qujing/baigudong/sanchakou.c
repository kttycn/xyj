...
int timer_sit(object ob)
{
  ...
  where->set("sanda_has_sitted",1);
  where->set("sanda_hero",ob);
...
int valid_leave (object who, string dir)
{
	...
	else if (who->query("obstacle/sanda")) return ::valid_leave(who,dir);
	lingfu=present("lingfu",who);
	if(!lingfu||!who->query_temp("obstacle/sanda_lingfu_getted"))
		return notify_fail("突然一阵瘴雾扑来，你一个踉跄，摔到在地上！\n");
		destruct(lingfu);
	who->delete_temp("obstacle/sanda_lingfu_getted");
