...
void greeting(object ob)
{
  ...
  {
    bool_var=0;
    if(ob->query("obstacle/sanda")=="done") return;
    if(ob->query_temp("obstacle/sanda_nuzi_killed")!=0) bool_var=1;
    if(ob->query_temp("obstacle/sanda_furen_killed")!=0) bool_var=1;
    if(ob->query_temp("obstacle/sanda_gonggong_killed")!=0) bool_var=1;
    if(bool_var==1)
     {
...
void unconcious()
{
  ...
  if (ob)
  {
    ob->set_temp("obstacle/sanda_baigujing_killed",1);
