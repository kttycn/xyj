...
void check_hair ()
{
	...
	if (me->query("disable_type") == "<睡梦中>" //&& me->query_temp("disabled")
	)
	{
...
{
	object me = this_object();
	message_vision ("$N说道：不期今夜皈依，教朕等为僧。\n\n",me);
...
void announce_success (object who)
{
	...
	who->add("score", 10);
	command("chat "+who->query("name")+"灭法国劝朕皈依佛门，朕改国号为钦法国！");
	message("channel:chat",HIW"【过关斩将】国王(Guo wang)："+who->query("name")+"闯过西行取经第三十关！\n"NOR,users());
