...
void announce_start()
{
	...
	int i;
	command("chat 各位施主请了！");
	command("chat 降魔宝籍今日出世，请各位有缘人前来一试。");
	user = users();
	for(i=0; i<sizeof(user); i++) {
		user[i]->set_temp("book_give_can","done");
		user[i]->save();
	}
}
...
void announce_finish(object ob)
{
	...
	user = users();
	for(i=0; i<sizeof(user); i++) {
		user[i]->delete_temp("book_give_can");
		user[i]->save();
	}
}
...
{     
	...
	if(who->query("obstacle/huangfeng") != "done" ) {
		message_vision("$N笑道，时候未到，这位施主且回！\n",ob);
		return 1;
	}
	...
	message_vision("$N从怀中取出本《伏魔山心经》递给$n，微笑道，但望施主不负所托，降魔卫道！\n", ob,who);
	command("chat "+who->query("name")+"施主福缘深厚，获得伏魔山心经一本！");
	remove_call_out("announce_finish");
	call_out("announce_finish",60,ob);
	return 1;
}
...
void announce_success (object who)
{
	...
	message("channel:chat",HIW"【过关斩将】灵吉菩萨(Lingji pusa)："+who->query("name")+"顺利闯过西行取经第六关！\n"NOR,users());
