...
int do_drink (string arg)
{
	...
	if (who->query("family/family_name")!="陷空山无底洞")
		return notify_fail ("你又不是老鼠，喝香油干什么？\n");
	if (!query("liquid/remaining"))
		return notify_fail ("葫芦里已经没有酥合香油了。\n");
	else
	{
		message_vision("$N拿起$n咕噜噜地喝了几口酥合香油。\n",who, me);
		who->set_temp("wdd_drink_hulu", 1);
