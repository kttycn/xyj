...
int do_clear() {
	...
	for (int i=0;i<sizeof(env);i++)
		if( (env[i]!=me && userp(env[i]) && env[i]->query("daoxing")<50000 ) ) {
//			message_vision("$N符合条件。\n",env[i]);
//			zhenyuan->send_back(zhenyuan,env[i]); }
			remove_call_out("alternative_die");
			call_out("alternative_die", 1, env[i]); }
	return 1;
}
