...
void appear(object ob)
{
	...
	message_vision(HIW"\n忽然清风、明月闯了进来。\n"NOR,ob);
	message_vision(HIR"清风破口大骂：$N,这个混蛋，偷我们五庄观的人参果。\n"NOR,ob);
	message_vision(HIR"明月破口大骂：$N,你比猪还馋吗？\n"NOR,ob);
	message_vision(HIR"清风破口大骂：$N,你这个贼子，一家人都是贼，才生出这样的贼人。\n"NOR,ob);
	message_vision(HIR"明月破口大骂：$N,快把人参果从肚子里给我吐出来。\n"NOR,ob);
	message_vision(HIR"清风破口大骂：$N,你是不是平时也是偷鸡摸狗的货色。\n"NOR,ob);
	message_vision(HIR"明月破口大骂：$N,我看你是一身的贼骨头。\n"NOR,ob);
	message_vision(HIR"清风破口大骂：$N,你一定不得好死。\n"NOR,ob);
	message_vision(HIR"明月破口大骂：$N,他妈的，别怪老子不客气了。\n"NOR,ob);
}
...
void kill(object ob)
{
	...
	message_vision(CYN"只见$N气的满脸通红，真是个怒发冲冠。\n",ob);
	message_vision(CYN"$N抡起手中的"+wield->query("name")+"对着人参果树狠命的砸了下去。\n"NOR,ob);
	message_vision(HIW"可怜一棵千年古树就这样慢慢的枯萎了。\n"NOR,ob);
	message("channel:chat",HIC"【闲聊】明月(Ming yue)：不好了！"+ob->query("name")+"把人参果树推倒了。\n"NOR,users());
