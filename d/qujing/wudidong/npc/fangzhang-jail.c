...
int do_subtract(string arg)
{
	...
	if(arg != "str" && arg != "cor" && arg != "int" && arg != "spi"
	&& arg != "cps" && arg != "per" && arg != "con" && arg != "kar")
...
int do_state(string arg)
{
	...
	while(i--)
	{
		if((count=points[gift_name[i]]) > 0)
			msg+="你准备将"+gift[gift_name[i]]+"增加"+chinese_number(count)+"点。\n";
		else if(count < 0)
			msg+="你准备将"+gift[gift_name[i]]+"降低"+chinese_number(-count)+"点。\n";
		continue;
	}
	tell_object(me, msg);
	return 1;
...
*/
	...
	me->set("obstacle/wudidong","done");
	me->delete_temp("mark/wudidong_opened_door");
	i=random((mykar-10)*80);
	me->add("daoxing",i+4000);
	me->add("score", 10);
	tell_object (me,"你赢得了"+COMBAT_D->chinese_daoxing(i+4000)+"的道行！\n");
	command("rumor 听说"+me->query("name")+"独闯无底洞，救出了玉鼠精抓去的一个老和尚，也不知是真是假。");
	me->save();
	return 1;
}
void destroy(object ob)
