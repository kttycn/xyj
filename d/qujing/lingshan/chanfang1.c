...
这里空荡荡的，只在靠近墙壁的地方整整齐齐地放着一些
蒲团。房里光线很暗，墙壁不知道是什么材料做的，会发
出淡淡的黄光。这里是坐禅(zuochan)参悟的好地方。你
要是实在定不下心也可以出去走走再回来(halt)。
...
void reward(object me)
{
        ...
        if ( kind == "杀" ) kind_bonus=300*3;
        else if ( kind == "擒" ) kind_bonus=400*3;
        else if ( kind == "劝" ) kind_bonus=500*3;
...
//	extra_bonus = (me->query("combat_exp")/1000)/4*2;   // yxm *2
        ...
        str = me->name()+"完成了一个难度"+chinese_number(level)+"的"+kind+"妖任务，\n" + str;
        str = str + "用时"+time+"。\n";
