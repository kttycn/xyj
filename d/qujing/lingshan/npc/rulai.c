...
//string *list=({"yg-wzg","yg-hell","yg-hyd","yg-jjf","yg-wdd","yg-fc",
//        });
...
void informing (object me, object who)
{
	...
	who->set("master/obstacle",HIY"斗战胜佛"NOR);
	me->set("my_level",LEVEL_NONE);
	me->delete_temp("invite");
	announce (me,who->query("name")+"汝因大闹天宫，吾以甚深法力，压在五行山下，幸天
灾满足，归于释教，且喜汝隐恶扬善，在途中炼魔降怪有功，全终全始，加升大职正果，
汝为"HIY"斗战胜佛"HIC"！\n");
	who->add("score", 800);
...
int reward_player ()
{
  ...
  if ((i/4-j) > 0)
  {
    message_vision ("$N对$n摇摇头：月影奇缘现有"+chinese_number(i)+"位玩家，"
                    "此处仅有数位仙贤在邀，可否再邀"+chinese_number(i/4-j)+
                    "位仙贤前来观礼？\n",me,who);
    if (! DEBUG)                 
      return 1;
  }
  announce (me,"月影奇缘西天取真经观礼："+who->query("name")+"大雄宝殿拜礼！\n");
...
//             }
        if( me->query("rulaitask/get") )
             {message_vision(CYN"\n如来佛说道：我不是已经给了任务你吗，快快起程，降妖除魔，功德无量。\n"NOR,me);
              return 1;
             }
...
很容易会令自身堕入魔道的，你这就去禅房好好参悟刚才的战斗去吧。\n"NOR,me);
        ...
        guai = new("/obj/rulai/npc/"+list[random(sizeof(list))]);
