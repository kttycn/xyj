...
me->delete("env/brief_message");
	...
	message_vision(HIY"\n$N身型闪动，地面随之起伏一降狂风，手中"+weapon->name()+NOR+HIY"，也有如狂风暴雨般地向$n卷来！\n"NOR,me,target);
	  ...
	  if (random(i)>random(j)) {
		damage=k/400+(int)me->query("str")/2;//原来是k,*7
		target->receive_damage("kee",damage);
		target->receive_wound("kee",damage/50);//damage/8
...
//		COMBAT_D->do_attack(me, target, weapon);
	...
	if (random(i)> random(j)) {
		message_vision(HIY"\n$N一杖拍出，马上又是一杖，刺向$n！\n"NOR,me,target);
		damage=k/400+random(k)/200+(int)me->query("str")/2;//原来是K
		target->receive_damage("kee",damage);
		target->receive_wound("kee",damage/50);//damage/6
		p = (int)target->query("kee")*100/(int)target->query("max_kee");
...
//		COMBAT_D->do_attack(me, target, weapon);
	...
	if (random(i)> random(j)) {
		...
		damage=k/400+random(k)/200+(int)me->query("str")/2;//k+random(k)*2+(int)me->query("str")*8
		target->receive_damage("kee",damage);
		target->receive_wound("kee",damage/50);//damage/8
		p = (int)target->query("kee")*100/(int)target->query("max_kee");
		msg = damage_msg(damage, "砸伤");
...
//		COMBAT_D->do_attack(me, target, weapon);
	...
	if (random(2)) {
		...
		damage=k/400+random(k)/200+(int)me->query("str")*1;//damage=k+random(k)*3+(int)me->query("str")*8;
		target->receive_damage("kee",damage);
		target->receive_wound("kee",damage/50);//damage/5
		p = (int)target->query("kee")*100/(int)target->query("max_kee");
		msg = damage_msg(damage, "砸伤");
		msg += "( $n"+eff_status_msg(p)+" )\n";     
		message_vision(msg, me, target);
