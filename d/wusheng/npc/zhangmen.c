...
void create()
{
	set_name("乾坤尊者", ({ "qiankun zunzhe", "zunzhe", "zhang men", "zhangmen" }) );
	set("gender", "男性" );
	set("age", 30);
	set("str", 30);
	set("per", 30);
	set("int", 30);
	set("long", "终南山武圣门掌门大弟子。" );
...
void init()
{
	...
	me->create_identity("/d/wusheng/npc/xianglong", "/d/wusheng/room1");
...
void find_master (object me, object ob)
{
	...
	{
		message_vision ("$N见了赶紧下拜，又抬起头朝$n使了个眼色。\n\n",me,ob);
...
{
	CHANNEL_D->do_channel(who,"chat",HIC"奉祖师号令：本派掌门弟子"+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
