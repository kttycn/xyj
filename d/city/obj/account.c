...
void create()
{
	set_name("信用卡", ({ "credit card","credit","card" }) );
	set("unit", "张");
	set("long",@LONG
这是月影奇缘世界中通用的新式信用卡，上面记录着你在银行中的
存款。望妥善保管，遗失后果自负。
...
int do_takeout(string arg)
{
	...
	if (!money_ob)
	{
		if(file_size("/obj/money/"+money+".c")<0) return notify_fail("没有这种货币!\n");
...
int do_info(string arg)
{
	...
	else
	{
		write("目前, 你在银行里有:\n");
		if (rich/1000000)
			write( chinese_number(rich/1000000) +"枚月影金币");
		rich=rich%1000000;
		if (rich/10000)
