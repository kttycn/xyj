...
void pay_player(object me, int total)
{
	object pay_jinbi, pay_cash, pay_gold, pay_silver;

	if(total/10000 > 0)
	{
		pay_jinbi = new("/obj/money/gold-coin");
		pay_jinbi->set_amount (total/10000);
		total -= 10000*pay_jinbi->query_amount();
...
{//this function check the money in silver a player has,
	...
	silver = present("silver_money", me);
	total_silver = 0;
	if( jinbi ) total_silver += 10000*(jinbi->query_amount());
