...
void create()
{
	set_name(MAG"菲飞"NOR, ({MAG"zong guan"NOR, MAG"zongguan"NOR, MAG"fei"NOR}));
	set("long", "他是发送装备的总管"MAG"菲飞"NOR",你可以(list)查看想要的东西。\n");
	set("nickname", CYN"落一叶而知天下秋"CYN);
	set("gender", "女性");
	set("age", 20);
	set("per", 50);
	set("title",HIR"装备总管"NOR);
	set_skill("literate", 141);
	set("chat_chance",20);
    set("chat_msg", ({
        MAG"菲飞"NOR"轻声告诉你：你可以用"HIY"list"NOR"指令列出想要的东西！\n",
        MAG"菲飞"NOR"轻声告诉你：你可以用"HIY"yao xxx"NOR"指令向大总管我索取装备。\n",
        MAG"菲飞"NOR"很神气的说：我就是这里的大总管，大总管就是我！\n",		
        }));
	setup();
...
int do_fight(string arg)
{
	...
	{
		message_vision("$N看样子想打架，但"MAG"菲飞"NOR"很有涵养,不和$N动粗。\n",who,me);
		return 1;
	}
	return 0;
}
...
int do_kill(string arg)
{
	...
	if (arg && present(arg,environment(who))==me)
	{
		message_vision("$N看样子想杀死"MAG"菲飞"NOR"，但"MAG"菲飞"NOR"是文化人,没跟$N一般见识。\n",who,me);
		return 1;
	}
	return 0;
}
...
int do_cast(string arg)
{
	...
	message_vision("$N对着"MAG"菲飞"NOR"结结巴巴的念了几句咒语，自己都感觉难听死了。\n",who,me);
...
int do_steal(string arg)
{
	...
	message_vision("$N想偷东西，但在"MAG"菲飞"NOR"面前，却总是有贼心没贼胆。\n",who,me);
...
int do_ji(string arg)
{
	...
	message_vision("$N想对"MAG"菲飞"NOR"祭法宝，前思后想，还是没敢。\n",who,me);
...
int do_throw(string arg)
{
	...
	message_vision("$N掏出暗器，看了看"MAG"菲飞"NOR"，信心不足，又把暗器收了起来。\n",who,me);
		...
		sscanf(arg,"%s",type);
			...
			case "armor":
				...
				ob->set("no_sell", 1);
        			...
        			ob->set("no_give", 1);
				ob->move(who);
				tell_object(who,MAG"菲飞"NOR"把"+ob->query("name")+"交给你,并告诉你:带好，别乱丢哦！\n");
				break;
			case "pifeng":
