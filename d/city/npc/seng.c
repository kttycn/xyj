...
string ask_age()
{
	...
	no = sizeof(ob) + 1;
	return me->name()+RANK_D->query_respect(me)+"的年龄在月影奇缘目前中排名第"
		+chinese_number(no)+"。";
}
string ask_beauty()
{
	...
	no = sizeof(ob) + 1;
	return me->name()+RANK_D->query_respect(me)+"的容貌在月影奇缘目前中排名第"
		+chinese_number(no)+"。";
}
int ask_force()
{
	...
	no = sizeof(ob) + 1;
	command("say "+me->name()+RANK_D->query_respect(me)+"的内力在月影奇缘目前中排名第"
		+chinese_number(no)+"。");
	return 1;
}
int ask_spells()
{
        ...
        no = sizeof(ob) + 1;
        command("say "+me->name()+RANK_D->query_respect(me)+"的法力在月影奇缘目前排名第"
                +chinese_number(no)+"。");
        return 1;
}
int ask_pk()
{
        ...
        no = sizeof(ob) + 1;
        command("say "+me->name()+RANK_D->query_respect(me)+"的杀害玩家数在月影奇缘目前排名第"
                +chinese_number(no)+"。");
        return 1;
}
int ask_mk()
{
        ...
        no = sizeof(ob) + 1;
        command("say "+me->name()+RANK_D->query_respect(me)+"的杀害ＮＰＣ数在月影奇缘目前排名第"
                +chinese_number(no)+"。");
        return 1;
}
int ask_exp()
{
        ...
        no = sizeof(ob) + 1;
        command("say "+me->name()+RANK_D->query_respect(me)+"的武学在月影奇缘目前在线玩家中排名第"
                +chinese_number(no)+"。");
        message_vision(CYN "$N指着$n赞叹道：“"+me->name()+"是月影奇缘中第"
...
int ask_daoxing()
{
        ...
        no = sizeof(ob) + 1;
        command("say "+me->name()+RANK_D->query_respect(me)+"的道行在月影奇缘目前在线玩家中排名第"
                +chinese_number(no)+"。");
        message_vision(CYN "$N指着$n赞叹道：“"+me->name()+"是月影奇缘中第"
                +chinese_number(no)+"高手！”\n" NOR, this_object(), me);
        return 1;
