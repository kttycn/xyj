//×î³õÀ´×ÔÌ¨Íå£¬µ«ËÆºõÒÑ±»»ÄÁ¹£¨wiman£©¸ÄµÃÃæÄ¿È«·Ç£¡
//		AIÎÒÖ®×·Çó£¬Ï£Íû´ó¼Ò¿ìÀÖ¡£
...
int dump_samecolor( string str_out, string str_mj, object me, mapping );//×öÇåÒ»É«
...
string *MjC = ({"",
                "Ò»Íò","¶şÍò","ÈşÍò","ËÄÍò","ÎéÍò","ÁùÍò","ÆßÍò","°ËÍò","¾ÅÍò",
                "Ò»Í²","¶şÍ²","ÈşÍ²","ËÄÍ²","ÎéÍ²","ÁùÍ²","ÆßÍ²","°ËÍ²","¾ÅÍ²",
                "Ò»Ìõ","¶şÌõ","ÈşÌõ","ËÄÌõ","ÎéÌõ","ÁùÌõ","ÆßÌõ","°ËÌõ","¾ÅÌõ",
...
int DELAY =0;			//NPCÃşÅÆ»ò³öÅÆµÄÑÓÊ±
string TIMES = "";		//¿´¶«ÄÏÎ÷±±·çµÄ


void create()
{
set_name(HIC"°×¾§¾§"NOR,({"bai jingjing", "bai", "jingjing", "jing"}));
        set_weight(1);
        set("gender", "Å®ĞÔ");
        set("age", 18);
        set("long",
                "Ëı¾ÍÊÇ°×¾§¾§£¬Äã¿ÉÒÔÎÊËı¹ØÓÚÂé½«µÄÎÊÌâ£¨ask bai about mj£©¡£\n"
                );
...
int do_addin()
{
        ...
        ||  me->query("id")==P4Data["Id"]){
            ...
            else message_vision("$N¶Ô×Å´ó¼ÒËµ½ñÌì×ÔÃş"+(random(12)+1)+"´Î¾ÍºÃ¡£\n",me);
            return 1;
        }
       if (Playing){
                if (random(1))
...
int do_dump(string str,object me)
{
        ...
        tell_object(present(NowPlayerId[NowPlayer]), "°×¾§¾§¶ÔÄãËµ£ºÏÖÔÚÂÖµ½ÄãÃşÅÆ¡£\n");
...
int do_gon(string str,object me)
{
        ...
        if (i < 3) return notify_fail("ÄãÊÖÖĞÒªÓĞÈşÕÅ"+do_check_num(str,0)+"²ÅÄÜ¸Ü¡£\n");
        if (!Touched){
...
varargs int do_win(string str,object me)
{
        ...
        if (do_check_win(sort_data(who["Mj"]))==1||i==1){//ÎªÁË·ûºÏÓÒ±ß·ÅÅÆµÄÏ°¹ß¡­¡­ËùÒÔÕâÀïÒªÖØĞÂsort_data
//ËãÌ¨///////////////////////////////////
        mixed MjT = ([
...
"@j" : ({ "Èş°µ¿Ì", 2 }),"@H" : ({ "¶şØ§¿Ú", 2 }),"@k" : ({ "Èş¸Ü×Ó", 2 }),"@B" : ({ "´ºÏÄÇï¶¬", 2 }),
"@C" : ({ "Ã·À¼¾ÕÖñ", 2 }),"@l" : ({ "ÃÅÇå×ÔÃş", 3 }),"@m" : ({ "ÅöÅöºú", 4 }),"@n" : ({ "»ìÒ»É«", 4 }),
"@o" : ({ "Ğ¡ÈşÔª", 4 }),"@p" : ({ "ËÄ°µ¿Ì", 6 }),"@q" : ({ "ÇåÒ»É«", 8 }),"@r" : ({ "×ÖÒ»É«", 8 }),
"@s" : ({ "Æß¶Ô×Ó", 8 }),"@t" : ({ "´óÈşÔª", 8 }),"@u" : ({ "Îå°µ¿Ì", 8 }),"@F" : ({ "Ğ¡ËÄÏ²", 8 }),
...
int do_pon(string str,object me)
{
        ...
        if(userp(me)) tell_object(me, "ÒÔÏÂÊÇÄãµÄÅÆ:\n");
        i=0;
        if (who["Show"]=="´úÂë") i=3;
        if (who["Show"]=="color") i=5;        
...
int do_restart(string str)
{
        ...
        message_vision("$NËµµÀ£ºÅÆ¾ÖÖØĞÂÉè¶¨ÇëÓÃ"+HIR+" addin "+NOR+"¼ÓÈëÅÆ¾Ö¡£\n",this_object());
...
//        for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
	PAUSE = 0;
        ...
        message_vision("$NËµµÀ£º×¯¼Ò"+HIY+NowPlayerId[0]+NOR+"Çë³öÅÆ¡£\n",this_object());
...
int do_setmj(string str)
{
        ...
        if (str=="tin"){
           ...
           if (check < 3) return notify_fail("ÄãÊÖÖĞÒªÓĞÈşÕÅ"+do_check_num(numstr,0)+"²ÅÄÜÉè¶¨¸Ü´ËÅÆ¡£\n");
        ...
        if (arg=="1p"||arg=="2p"||arg=="3p"||arg=="4p"){
           ...
           Mj+="ÄãµÄÉè¶¨ÓĞ:";
              ...
              tell_object(me, "ÏÖÔÚÊÇ["+TIMES+"] :×¯¼Ò "+HIY+present(NowPlayerId[0])->query("name")
              	+"("+NowPlayerId[0]+")"+NOR+"\n" );
              Mj+=HIG+"¶«×À  "+NOR+present(P1Data["Id"])->query("name")+"("+P1Data["Id"]+")"+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";              
              Mj+=show_mj(next_data(P1Data["Id"], 0, "allout"),i+1);              
              Mj+=HIG+"ÄÏ×À  "+NOR+present(P2Data["Id"])->query("name")+"("+P2Data["Id"]+")"+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";
              Mj+=show_mj(next_data(P2Data["Id"], 0, "allout"),i+1);             
              Mj+=HIG+"Î÷×À  "+NOR+present(P3Data["Id"])->query("name")+"("+P3Data["Id"]+")"+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";
              Mj+=show_mj(next_data(P3Data["Id"], 0, "allout"),i+1);                                         
              Mj+=HIG+"±±×À  "+NOR+present(P4Data["Id"])->query("name")+"("+P4Data["Id"]+")"+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";
...
int npc_action( object me )
{
        ...
        for(i=0; i<3; i++) {
        	...
        	if( do_check_win(who["Mj"]) ) {        		
        		if( !userp(ob) ){
        			...
        			tell_object(ob, "ÊÇ·ñºú»òÅö»ò¸Ü"+ do_check_num(LastDump, 0)+"£¿£¿£¨Èç¹ûÊÇ£¬ÇëÊäÈë£ºhu/pon/gon£©\nÈç¹û·ñ£¬ÇëÊäÈëcontinue\t");
	...
	//for(i=0; i<3; i++) {
        	...
        	if( !userp(ob) ) {
        		...
        		if( do_check_Mj(next_data(me->query("id"), i+1, "Mj"), LastDump)>2 ){
        			...
        			PAUSE = 2;	//°ÑÊ±¼äÅªÍ££¬·ÀÖ¹Íæ¼Ò¶ÔÕ½Ê±ÇÀÃş¡­¡­
        			tell_object(ob, "ÊÇ·ñÅö»ò¸Ü"+ do_check_num(LastDump, 0)+"£¿£¿£¨Èç¹ûÊÇ£¬ÇëÊäÈë£ºpon/gon£©\nÈç¹û·ñ£¬ÇëÊäÈëcontinue\t");
        			who["Pause"] = "yes";        			
        			return 1;
        		}
        			...
        			tell_object(ob, "ÒÔÏÂÊÇÄãµÄÅÆ£º\n"+show_mj(next_data(me->query("id"), i+1, "Mj"), 6) );
        			PAUSE = 2;	//°ÑÊ±¼äÅªÍ££¬·ÀÖ¹Íæ¼Ò¶ÔÕ½Ê±ÇÀÃş¡­¡­
        			tell_object(ob, "ÊÇ·ñÅö"+ do_check_num(LastDump, 0)+"£¿£¿£¨Èç¹ûÊÇ£¬ÇëÊäÈë£ºpon£©\nÈç¹û·ñ£¬ÇëÊäÈëcontinue\t");
        			who["Pause"] = "yes";        			
        			return 1;
        		}
       		}
	...
	if( userp(ob) ) {
		...
		{
			tell_object( ob, "Äã¿ÉÒÔÓÃ"+HIR+"eat"+NOR+"À´³ÔÅÆ£¡£¡\n");
			tell_object(ob, "ÒÔÏÂÊÇÄãµÄÅÆ£º\n" +show_mj( str, 6) );
		}
			return 1;
	}
