...
在开始你的取经历程之前，首先要为自己所创造的人物选择一个合适
的天赋，因为这将对你今后的发展有重大的影响。月影奇缘中的人物
天赋共有四项：

	膂力：影响攻击能力及负荷量的大小。
	悟性：影响学习武功的速度及理解师傅的能力。
	根骨：影响体力恢复的速度及增加的气血。
	灵性：影响法术的能力及增加的精神。

每项由一个十到三十之间的整数来表示，一般数值越大越好，但各项
的总和是固定不变的。
LONG
	);
	get_start(me);
...
void do_finish(object me)
{
	...
	*/
	...
	me->set("daoxing",500);
	me->set("combat_exp",500);
	me->delete("no_gift");
//	CHANNEL_D->do_channel( this_object(), "sys",
//		sprintf("%s由%s连线进入。", me->name()+"("+capitalize(me->query("id")) + ")", query_ip_number(me)) );
	me->flush_buffer(); // clear receive message buffer.
	tell_object(me,"\n\n\n好！祝你西天取经早成正果！\n\n\n");
	tell_object(me,"只见眼前霞光一闪，你已经来到了一个陌生的世界（ｌｏｏｋ）．．．\n\n");
	if(me->query("gender") == "男性"&&me->query("age")<=25)
	{
		switch(random(2))
        {
        	case 0 : str = "少年英雄";break;
			case 1 : str = RANK_D->query_respect(me);break;
		}
	}
	else if(me->query("gender") == "女性"&&me->query("age")<=25)
	{
		switch(random(2))
		{
	...
	else str = RANK_D->query_respect(me);

	if(!(wizardp(me)&&me->query("env/no_login_msg")))
		message("channel:chat", MAG"【月影奇缘】"NOR": "CYN"听说来了一位叫做"+HIW+me->name()+NOR CYN"的"+str+"。\n"NOR,users());

	if (("/adm/daemons/band"->create_char_banned(query_ip_number(me)) == 1 ||
	"/adm/daemons/band"->is_banned(query_ip_number(me)) == 1) && !wizardp(me))
	{
		me->move("/d/wiz/guest");
