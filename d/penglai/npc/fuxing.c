...
void die()
{
	object jp = new("/d/nanhai/obj/jp2");
	object me = find_living(this_player()->query_temp("last_fainted_from"));
	if( environment() ) {
		message("sound", "\n\n只听一声鹤鸣，福星化作一道白光向天上直冲而去。。。\n
半空中有人哈哈笑了几声。。。\n\n", environment());
		if (me->query_temp("ask_jingping") )
		{
			tell_object(me, CYN"福星走得匆忙，竟从他的大袖里掉出一个净瓶来，你眼明手快，一把接住，连忙放入怀中。。。\n\n"NOR);
			me->delete_temp("ask_jingping");
			me->set_temp("kill_fuxing", 1);
			jp->move(me);
		}
	}
