...
{
	if ( dir == "enter"){
		if(objectp(present("shou xing", environment(me))) && !wizardp(me))
			return notify_fail("寿星伸手拦住你：里面是老夫卧室，你去干什么？\n");
        	if(objectp(present("fu xing", environment(me))) && !wizardp(me))
                	return notify_fail("福星伸手拦住你：里面是老夫卧室，你去干什么？\n");  
        	if(objectp(present("lu xing", environment(me))) && !wizardp(me))
