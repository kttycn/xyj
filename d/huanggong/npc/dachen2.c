...
string *degree_desc = ({
       ...
       GRN "从九品上 文林郎" NOR,//90
       CYN "从九品上 陪戎校尉" NOR,
       CYN "正九品下 登仕郎" NOR,//150
       GRN "正九品下 怀化执戟长上" NOR,//180
       CYN "正九品上 仁勇副尉" NOR,
       RED "正九品上 仁勇校尉" NOR,
       GRN "正九品上 儒林郎" NOR,
       YEL "从八品下 承务郎" NOR,//300
       MAG "从八品下 归德司戈" NOR,
       CYN "从八品下 御侮副尉" NOR,
       RED "从八品上 承奉郎" NOR,
       GRN "从八品上 御侮校尉" NOR,
...
int ask_for_officer()
{
	...
	if ( ob->query("retire") == 1)
//			return notify_fail("你是怎么得到这把剑的?!哼！\n");
//	if ( ob->query("retire"))
	{
		command("say "+ob->query("name")+"不是告老还乡了吗？还是回家好好休息吧。\n");
		return 1;
	}
...
//	if ( ob->query("retire") == 1)
//	{
//		command("say "+ob->query("name")+"归隐前，户部不是已经给你发放了一笔返乡费吗？狗胆包天，竟然敢来讹本大人。\n");
//		command("heng");
//		return 1;
