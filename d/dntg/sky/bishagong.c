...
void init()
  {
   ...
   if (who->query("dntg/laojunlu")=="allow1")
     ...
     if (! present ("yisheng zhenjun",where))
       ...
       zhenjun->command("chat "HIR+who->query("name")
          +HIC" 这 "HIY+RANK_D->query_rude(who)+HIC" 打到毗沙宫了！"HIG"四大天王"HIC"快来救驾 ！！！"NOR);
       message_vision(HIG"只见四大天王蜂拥而来，将$N围在当中！！！\n"NOR,who);
       for (i=0;i<4;i++)
         {
...
int valid_leave(object me, string dir)
{
   ...
   if (me->query("dntg/laojunlu") != "allow1")
      ...
      return notify_fail(HIC"这么多人围着你，你哪也去不了！！！\n"NOR);
   else if (dir != "north")
