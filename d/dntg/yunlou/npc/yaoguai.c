...
int dx_reward, exp_reward, pot_reward;
...
void setname1()
{
	...
	name=names1[i]+names2[j]+names3[k];
        id=({this_player()->query("id")+"'s "+names1[i+1]+names2[j+1]+" "+
...
void setname2()
{
    ...
    name=names4[i]+(j?"精":"怪");
    id=({this_player()->query("id")+"'s "+names4[i+1]+" "+(j?"jing":"guai"),
	    (j?"jing":"guai")});
...
string _invocation(object who, int level)
{
	...
	if(yaoguai_type==1) {
		...
		long="一只"+query("name")[0..<3]+"，在"+({"深山中","密林中","野外",
			"草原上","野林中","高山中","莽林中","莽原上","深谷中",
			})[random(9)]+"汲取"+({"日月","天地","万物"})[random(3)]+
			"精华，颇有了些道行。\n";
		set("long",long);
...
//  if (!random(3)) set("title","米国鬼子")
	...
	if(exp<30000) {
		exp_reward=40+exp/60;
		pot_reward=16+exp/300;
	} else if(exp<300000) {
		exp_reward=80+exp/600;
		pot_reward=24+exp/6000;
		dirs=dirs+dirs2;
	} else if(exp<3000000) {
		exp_reward=120+exp/6000;
		pot_reward=27+exp/9000;
		dirs=dirs+dirs2;
	} else if(exp<5000000) {
                exp_reward=120+exp/6000;//原来exp_reward=160;NO.2exp_reward=120+exp/48000;
                pot_reward=27+exp/9000;//原来pot_reward=40;
		dirs=dirs+dirs2+dirs3;
	} else if(exp<8000000) {
		exp_reward=120+exp/12000;//exp_reward=160;原来
                pot_reward=27+exp/9000;//pot_reward=40;原来
		dirs=dirs+dirs2+dirs3;
	} else if(exp<11000000) {
		exp_reward=120+exp/12000;//exp_reward=160;原来
                pot_reward=27+exp/11000;//pot_reward=40;原来
		dirs=dirs+dirs2+dirs3;	
	} else if(exp<13000000) {
		exp_reward=120+exp/24000;//exp_reward=160;原来
                pot_reward=27+exp/13000;//pot_reward=40;原来
		dirs=dirs+dirs2+dirs3;	
	} else if(exp<21000000) {
		exp_reward=120+exp/30000;//exp_reward=160;原来
                pot_reward=27+exp/13000;//pot_reward=40;原来
		dirs=dirs+dirs2+dirs3;	
	} else if(exp<29000000) {
		exp_reward=120+exp/35000;//exp_reward=160;原来
                pot_reward=27+exp/20000;//pot_reward=40;原来
		dirs=dirs+dirs2+dirs3;	
	} else {
		exp_reward=220+exp/60000;
		pot_reward=140+exp/30000;
		dirs=dirs+dirs2+dirs3;
	}				
	pot_reward=pot_reward*(lvl+1);
	exp_reward=exp_reward*(lvl+1);
...
//            message_vision("$N眼见无奈，只好化成股青烟附到了$n的手中。\n", this_object(), owner_ob);
		...
		exp_r=exp_reward*ratio/80;
		pot_r=pot_reward*ratio/80;

		owner_ob->add("potential",pot_r);
		owner_ob->add("combat_exp",exp_r);
		owner_ob->add("score", 2);
		...
		tell_object(owner_ob, "你得到了"+chinese_number(exp_r)
			+"点武学经验和"+chinese_number(pot_r)+"点潜能！\n");

		if (owner_ob->query_temp("mieyao/level")==9 && ratio>50)
		{
			skill=give_reward(owner_ob);
			owner_ob->add("score", 10);
			if ( skill == "none")
				msg="听说"+ owner_ob->query("name")+ "的技能差点又要有所进境了！\n";
			else
			{
...
//			owner_ob->delete("mieyao/id");
//			owner_ob->delete("mieyao/name");
//			destruct(this_object());
//			MONITOR_D->report_system_object_msg(owner_ob, str2);
//			return;
		}
