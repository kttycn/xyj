...
int do_kill(string arg) {
	...
	message_vision("$N拍案大叫：“怎么又是"+name[next]+"？！你这奸王竟敢耍弄于我！拿命来！”\n\n",ob);
	...
	ob->add("dntg/number",1);
	ob->add("daoxing",i+5000);
	ob->add("score", 20);
	me->command("chat "+ob->name()+"这"+RANK_D->query_rude(ob)+
     "逞强行凶，不服拘唤，大闹罗森，强销名号，待我上天禀明玉帝，再来拿你！");
	tell_object(ob,"你赢得了"+COMBAT_D->chinese_daoxing(i+5000)+"的道行！\n");
	message("channel:rumor", HIW"【月影神迹】听说"+ob->name()+"("+ob->query("id")+")顺利闯过大闹天宫第三关 ！！！ \n\n"NOR, users());
	log_file("dntg",sprintf("%s(%s) passed hell on %s\n",ob->name(1), geteuid(ob), ctime(time()) ) );
