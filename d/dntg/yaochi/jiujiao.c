...
void announce(object me) {
	...
	message("channel:rumor", HIM + "【谣言四起】某人：听说"+
	me->query("name")+"溜进瑶池，偷喝了王母娘娘的仙酒。\n"NOR,users());
	me->add("con",1+random(2));
	me->add("dntg/number",1);
	me->set("dntg/yaochi","done");
	me->add("score", 20);
	message("channel:rumor", HIW"【月影神迹】听说"+me->name()+"("+me->query("id")+")顺利闯过大闹天宫第七关 ！！！ \n\n"NOR, users());
	log_file("dntg",sprintf("%s(%s) passed yaochi on %s\n",me->name(1), geteuid(me), ctime(time()) ) );
	me->save();
}
