...
int valid_leave(object me, string dir)
{
	...
	if(dir=="east"){
		...
		&& living(mys) && living(lys) && !wizardp(me) )
			return notify_fail("马流二元帅嘴里一阵吱吱乱叫，恶狠狠地把你拦住了。\n");
		if(objectp(mys=present("ma yuanshuai", environment(me)))&&living(mys)&&!wizardp(me))
			return notify_fail("马元帅嘴里一阵吱吱乱叫，恶狠狠地把你拦住了。\n");
		if(objectp(lys=present("liu yuanshuai", environment(me)))&&living(lys)&&!wizardp(me))
