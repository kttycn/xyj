...
{
  message_vision (HIR"看起来$n"HIR"想杀死$N！\n"NOR,me,who);
  me->kill_ob(who);
  who->kill_ob(me);
}
...
void kill_ob (object ob)
{
	...
	if( ob->query_temp("liuer-mihou_ask") )
		message_vision ("\n$N大怒：哪个泼贼子敢冒充你家猴爷！\n",this_player());
	else
		message_vision(HIR "\n看起来$n"HIR"想杀死$N！\n"NOR,this_player(), me);
...
void unconcious()
{
	...
	{
		ob->set_temp("liuer-mihou_killed",1);
		message_vision(HIC "观音说道：这厮我带走，你安心取经去吧。\n\n" NOR,me);
//		baoguo = new ("/d/obj/misc/baoguo");
//		baoguo -> move(ob);
		load_object("/obj/empty");
		me->move("/obj/empty");
//		message_vision("六耳弥猴拿出个包裹。\n\n",me);
		call_out ("guan_appearing", 1, ob, where);
	}
	ob->set_temp("liuer-mihou_ask","no");
	message_vision("六耳弥猴悻悻地走了。\n\n",me);
	call_out ("destruct_me",3,me);
}

void guan_appearing (object who, object where)
{
