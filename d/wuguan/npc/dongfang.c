...
int job_finish()
{
	...
	if(me->query_temp("wuguan/tool")) {
		tell_object(me,CYN"你先把工具还到孙管事那再来覆命吧。\n"NOR);
		return 1; }
...
CYN+"东方博玉对着$N说道：这位"+RANK_D->query_respect(me)+"已是本武馆的一员了，何苦开这种玩笑。\n"+NOR,me);
	...
	me->save();
	message_vision(CYN+"东方博玉告诉$N：正好我们这里最近招收江湖新手，"+RANK_D->query_respect(me)+
	"资质不错，就在我们这里干吧。\n"+NOR,me);
	message_vision(CYN"东方博玉对$N说道：从现在起你可以"NOR+HIY"(ask boyu about 效劳)"NOR+
	CYN"为武馆做些贡献。\n"NOR,me);
	return 1;
}
...
void greeting(object ob)
{
    ...
    if(ob->query("combat_exp")>30000 && ob->query("daoxing")>30000)
		return;
