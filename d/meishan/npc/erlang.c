...
void die()
{
	...
	&& query_temp("current_player"))
	{
		...
		message("channel:rumor", HIW"【月影神迹】听说"+who->name()+"("+who->query("id")+")顺利闯过大闹天宫第九关 ！！！ \n\n"NOR, users());
		log_file("dntg",sprintf("%s(%s) passed erlang on %s\n",me->name(1), geteuid(who), ctime(time()) ) );
	} 
