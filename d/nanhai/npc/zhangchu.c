...
string ask_zuofan()
{
	...
	if (who->query("family/family_name") != "ÄÏº£ÆÕÍÓÉ½")
		return RANK_D->query_respect(who) +"Óë±¾ÃÅºÁÎŞÔ¨Ô´£¬ÎªÊ²Ã´µ½ÕâÀ´×öÕ«·¹£¿\n";
	if (who->query("faith")>300||who->query("combat_exp")>50000)
		return RANK_D->query_respect(who) +"µÄÉí·İ³ç¸ß,²»ÒË×ö´ËµÈÔÓÊÂ!\n";
	who->set("make_dinner",1);
	return "ÕÆ³øÉ®¶ÔÄãÒ»Ò¾,µÀ:Èç´ËÓĞÀÍÊ¦µÜÁË¡£\n";
}
