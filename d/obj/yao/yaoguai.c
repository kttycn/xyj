...
void copy_status(object me,object ob, int lv)
{
	...
	set("daoxing",ob->query("daoxing")*lvl/20);
...
string _invocation(object who, int level,string loc)
{
	...
	if(exp<30000) {
		exp_reward=800+exp/60;
		pot_reward=300+exp/300;
	} else if(exp<300000) {
		exp_reward=1800+exp/600;
		pot_reward=400+exp/6000;
	} else if(exp<3000000) {
		exp_reward=2300+exp/6000;
		pot_reward=500+exp/60000;
	} else {
		exp_reward=2800+random(500);
		pot_reward=700+random(100);
	}
...
void check_room()
{
	...
	else if(!env=environment(me)) return;
	if(!living(me)) return;
...
void die()
{
	...
	if( stringp(owner) && objectp(owner_ob = find_player(owner)) ) {
		...
		if (owner_ob->query_temp("mpmieyao/level")==9) {
...
int strength()
{
	string* ycolor=({NOR,BLK,RED,YEL,BLU,MAG,CYN,WHT,HIR,HIG,HIY,HIB,HIM,HIC,HIW});
    int i;
