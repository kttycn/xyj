...
int obstacle_sc()
{
	...
	! me->query_temp("can_ask") )
		return notify_fail("我不太清楚。\n");
	...
	message("channel:chat",HIW"【过关斩将】阎罗王(Yanluo wang)："+me->query("name")+"闯过西行取经第二关！\n"NOR,users());
	tell_object(me, "你赢得了"+COMBAT_D->chinese_daoxing(i+4000)+"的道行！\n");
	me->save();
	me->move("/d/qujing/shuangcha/sleeproom");
	tell_object(me,"你揉揉眼睛，醒了过来。\n");
	return 1;
}
...
int rank_me()
{
	...
	if( me->query("title") == "大力鬼王")
		return notify_fail("你已经是掌门人了，还来要什么职位．\n");
	if( (string)me->query("family/family_name") == "阎罗地府"){
		if( exp <= 500000 ) {
			return notify_fail("你现在武学太低，还不能来要什么职位．\n");
		} else if(exp <= 1000000 ){
			me->set("nick", HIB"阎罗地府"NOR+RED"◆"BLU"勾魂小鬼"NOR);
		} else if(exp <= 2000000 ){
			me->set("nick", HIB"阎罗地府"NOR+RED"◆"YEL"变天无常"NOR);
		} else if(exp <= 5000000 ){
			me->set("nick", HIB"阎罗地府"NOR+RED"◆"RED"血手屠夫"NOR);
		} else if(exp <= 10000000 ){
			me->set("nick", HIB"阎罗地府"NOR+RED"◆"MAG"玄天鬼王"NOR);
		} else {
			me->set("nick", HIB"阎罗地府"NOR+RED"◆"CYN"阴司之主"NOR);
		}

		tell_object(me, "阎罗王对你说：你可以做"+me->query("nick")+"。\n");
		tell_room(environment(me),
"阎罗王对"+me->query("name")+"说：你可以做"+me->query("nick")+"。\n", ({me, me}));
...
int do_true(string arg)
{
	...
	this_player()->query_temp("betray")) {
		...
		if (this_player()->query_skill("gouhunshu"))
...
"/d/sea/", "/d/xueshan/", "/d/qujing/nverguo/",
...
void finish_check_book(object me,object master)
{
	...
	command("say 好弟子！");
	tell_object(me,HIY"你觉得对勾魂术有了一些领悟。\n"NOR);
	me->add("faith",1);
	me->add("combat_exp",9);
	me->add("potential",3);
