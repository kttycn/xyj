...
void create()
{
	set_name(HIC"龙神"NOR, ({ "long shen", "long", "dragon" }) );
	set("gender", "男性" );
	set("long",HIB"八部天龙\n"NOR);
...
void die ()
{
	...
	if( !ob ) {
		...
		message_vision (HIB"只见一道蓝光闪过，"NOR"$N"HIB"的精气神都恢复了\n"NOR,me,ob);
		return;
	}
	else {
		...
		message_vision ("$N"CYN"化作一股清风，不知去向......\n"NOR,me);
		destruct (this_object());
		return ;
	}
}
