...
int do_enter(string arg)
{
	object obj,me=this_player();
	object ob=this_object();
	if(this_player()->query_temp("fahua/enter") == "done") //!= "done"这是原来的
		return notify_fail("你不是已经进去了吗!!\n");
	if(!ob->query_temp("opened"))
...
{
//	message_vision("$N消失了... ...\n",ob);
	tell_room(environment(me), ob->query("name")+"渐渐变得透明了，一道豪光闪过，消失了... ...\n");
	ob->delete_temp("opened");
	ob->delete("exit");
	destruct(this_object());//destruct(me)
	return 1;
}
