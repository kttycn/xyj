...
void create()
{
  set_name(HIR"奔雷"+HIW"锏"NOR, ({ HIR"benlei jian"NOR, "jian", "mace" }) );
        set_weight(4000);
        if( clonep() )
...
int ji_ob(object victim)
{
        ...
        fabao->set("spi", 0);
         fabao->add("spi", 6000);
        if( me->is_busy() )
 return notify_fail("你正忙着呢，无法祭出"+HIR"奔雷"+HIW"锏。\n"NOR);
        if( !victim )
                return notify_fail("你想祭的人不在这里。\n");
        if( where->query("no_magic") )
  return notify_fail("这里不能祭"+HIR"奔雷"+HIW"锏。\n"NOR);
        if( !me->is_fighting() )
                return notify_fail("只有战斗中才能祭出。\n");
        if( (int)me->query("mana") < 800 )
 return notify_fail("你的法力不能控制"+HIR"奔雷"+HIW"锏。\n"NOR);

message_vision(YEL"\n$N大喝一声“去!”，只见$n"+YEL"「呼」地一下化做一条巨龙飞到半空。\n"NOR, me, fabao);

message_vision(YEL"霎那间天色一变！$n"+YEL"呼啸着向"+HIW"$N"+YEL"迎面击下！\n"NOR, victim, fabao);
