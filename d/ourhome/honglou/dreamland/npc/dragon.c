...
void unconcious()
{
	object killer,where,corpse;

	if( objectp(killer = query_temp("last_damage_from")) ) {
		set_temp("my_killer", killer->query("id"));
		COMBAT_D->killer_reward(killer,this_object());
		killer->set_temp("kill_qingsuo_dragon",1);
	}
	where = environment(killer);
//	::die();
	message_vision(RED"\n$N惨叫一身,但从尸体上却站起来一个僵尸！\n\n"NOR, this_object());
	if(!( corpse = find_object("/d/ourhome/honglou/dreamland/npc/corpse")) )
		corpse = load_object("/d/ourhome/honglou/dreamland/npc/corpse");
	if(objectp(corpse))
		corpse->move(where);
