...
void create()
{
	set_name(HIM"雷公"NOR, ({"lei gong", "lei", "gong"}));
	set("long", "一个耳朵很长的怪人，手中持一根铁棒。\n");
	set("gender", "男性");
	set("age", 70);
...
//	shengzhu_given = "/d/obj/baowu/shengzhu.c";
	if(objectp(me = query_temp("last_damage_from") ))
	{
		if ( (!leizhu_given->in_mud()) ) {
			...
			message_vision("$N扔下一颗碧蓝色的珠子，$n连忙放入怀中。\n",this_object(),me);
			zhu = new("/d/obj/baowu/leizhu");
			zhu->move(me);
			MONITOR_D->report_rumor_object_msg(me, "拿到雷灵珠了！");
		}
		message_vision(HIM"\n$N大喝一声：我去也。\n"NOR,this_object());
	}
	else {
		message_vision(HIM"只见$N大喊一声，想得到宝贝，你做梦。\n"NOR,this_object());
