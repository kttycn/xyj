...
mixed main(object me, string arg, int remote)
{
	...
	str = "\n■ "CYN"〖"+MAG+ CHINESE_MUD_NAME+CYN+"〗"HIY"·"NOR CYN+INTERMUD_MUD_NAME+NOR+HIW"   目前";
	if(opt_banghui)
	...
	if(opt_long || opt_wiz)
	{
		...
		while ( i-- )
		{
			...
			str = sprintf("%s%12s%s%s\n",str,RANK_D->query_rank(list[i]),
				interactive(list[i])?(query_idle(list[i]) > 120? HIY "@" NOR:" "):HIR "#" NOR,
				list[i]->short(1));
		...
		while( i-- ){
			...
			name=interactive(list[i])?(query_idle(list[i])>120?HIY"@"NOR + WHT:" "):HIR "#"NOR + WHT;
			if( wizardp(list[i])){
				name = sprintf( HIG"%s%-10s(%-11s"HIC"│"NOR + HIG,name,list[i]->name(1),
...
"HIY"@"NOR" 表示"HIG"发呆"NOR"中，"HIR"#"NOR" 表示"HIR"断线"NOR"中。
