...
int main(object me, string arg)
{
	...
	|| me->query("class")=="wusheng" 
	|| me->query("class")=="bonze" 
	|| me->query("class")=="taoism" )
	{
		if(me->query("daoxing") >= 5000000){
			titlelength=8;
		}
		else if(me->query("daoxing") >= 1000000){
			titlelength=6;
		}
		else if(me->query("daoxing") >= 500000){
			titlelength=4;
		}
	} else if( me->query("class")=="yaomo" 
	|| me->query("class")=="dragon" 
	|| me->query("class")=="youling" )
	{
		if((me->query("daoxing")+me->query("combat_exp"))/2 >= 5000000){
			titlelength=8;
		}
		else if((me->query("daoxing")+me->query("combat_exp"))/2 >= 1000000){
			titlelength=6;
		}
		else if((me->query("daoxing")+me->query("combat_exp"))/2 >= 500000){
			titlelength=4;
		}
	} else if(me->query("class")=="fighter" 
	|| me->query("class")=="swordsman" 
	|| me->query("class")=="hero" 
	|| me->query("class")=="phoenix" )
	{
