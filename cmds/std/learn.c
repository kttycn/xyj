...
int main(object me, string arg)
{
	...
	if(!arg) return notify_fail("指令格式：learn <技能> from <某人> for <次数>\n");
...
// don't have the skill.
	if(userp(ob) && (int)my_skill<1 && !ob->query("guild/child") ) {
		return notify_fail("可是你怎么也弄不明白。\n");
	}

// can't learn above 100 level from a player.
// mon 3/7/98
	if(userp(ob) && my_skill>=100 && !ob->query("guild/child") )
		return notify_fail("可是"+ob->name()+"已经无力指导你进一步提高了。\n");

	tell_object(ob, sprintf("%s向你请教有关「%s」的问题。\n",
		me->name(), to_chinese(skill)));

	sen1=(int)ob->query("sen");
	if( (userp(ob) || sen1<50) && !ob->query("guild/child") )
		if(sen1 > sen_cost + 1 ) {
			if( userp(ob) && !ob->query("guild/child") )
				ob->receive_damage("sen", sen_cost+1);
		} else {
...
//to learn beyond NPC master's skill with an added cost.
			...
			if( my_skill < master_skill ) {
				...
				me->improve_skill(skill, amount,(qlearned>4*qskill && !userp(ob))&& !ob->query("guild/child") ?0:1);
