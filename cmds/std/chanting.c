...
void finish(object me,int dx_gain, int pot_gain)
{
	int buddhism0, buddhism1, buddhism2;
	string *msg=({
	...
	if (me->query("food") >= pot_gain) me->add("food",-pot_gain*2);
	else me->set("food", 0);
	buddhism1 = (me->query_skills()["buddhism"]+1)*(me->query_skills()["buddhism"]+1);
	buddhism2 = me->query_learned()["buddhism"];
	
	me->receive_damage("sen", me->query("int"));
	me->receive_damage("kee", me->query("int"));
	buddhism0=random(me->query_skill("buddhism")*me->query_int()/10);
	me->improve_skill("buddhism", buddhism0, (buddhism1>buddhism2 && 
		me->query_skill("spells") > me->query_skill("buddhism")) ?0:1);
	tell_object(me, HIC "你的大成佛法增加了" + chinese_number(buddhism0) + "点！\n" NOR);
