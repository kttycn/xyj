...
int main(object me, string arg)
{
	...
	if( !arg )
	{
		if( !mapp(pmap) || sizeof(pmap)==0 )
			return notify_fail("你现在没有使用任何特殊攻击技能。\n");

		skill = keys(valid_types);
		write("以下是你目前使用的特殊攻击技能。\n");
	...
	if ( sizeof(pmap) >= 2 )
		return notify_fail("你现在已经使用了两种特殊技能。\n");
...
//				return notify_fail("这两种拳术技能不能组合。\n");
			...
			if ( !basic1 || !basic2 )
				return notify_fail("这两种技能尚未激发或目前不能准备。\n");
