...
int do_give(object me, object obj, object who)
{
	...
	if( !userp(who) && obj->value() ) { // give money to npc
		...
		if ( wizardp(me) && !wizardp(who) && userp(who) ) {
			log_file("static/give", sprintf("%s(%s) 给%s(%s) 一%s%s 时间 %s\n",
				me->name(1),geteuid(me),who->name(1),geteuid(who),unit,obj->name(),ctime(time())));
			message("channel:chat",HIR"【监察精灵】公正法官(Faguan)："+"据说"+me->query("name")+"给了玩家 "HIW+who->query("name")+
				HIR" 一"+obj->query("unit")+obj->name()+"\n"NOR,users());
		}
