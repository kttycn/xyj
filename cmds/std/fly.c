...
int main(object me, string arg)
{
	...
	if( !wizardp(me) && environment(me)->query("no_fly") )
		return notify_fail("这个地区没办法腾云驾雾。\n");
	if( me->is_ghost() )
...
notify_fail("你现在头脑不太清醒，当心掉下来摔死。\n");
	...
	if( (string)me->query("family/family_name") == "陷空山无底洞") {
	...
	} else if( (string)me->query("family/family_name") == "盱眙山") {
		message_vision(HIW"$N念动口诀，只见一个霹雳过后，$N竟然踪影全无。 \n"NOR, me);
	} else if( (string)me->query("family/family_name") == "凤凰星") {
		message_vision(HIR"$N摇身一变，现出原形，却是一只凤凰，充满伤感的火焰燃烧起来，$N走进火焰中消失了......\n"NOR, me);
		...
		message_vision(RED"$N捻个口诀，口中念动真言,突然“轰”地冒起一团红雾,$N随即隐入红雾中。\n"NOR, me);
	} else if( (string)me->query("family/family_name") == "蜀山剑派") {
		message_vision(HIC"$N捻个口诀，叫道“咄“，背上宝剑应声飞出，$N飞驰而上，一招「御剑飞行」向远方飞去。。。。\n"NOR, me);
	} else if( (string)me->query("family/family_name") == "盘丝洞") {
	...
	if(arg=="stone") loc="/d/dntg/hgs/entrance";
	...
	else if(arg=="xuyi") loc="/d/xuyi/nanmen";
	...
	else if(arg=="baigushan") loc="/d/qujing/baigudong/shanlu4";
	else if(arg=="huangfeng") loc="/d/qujing/huangfeng/shanlu1";
		...
		if(me->query("family/family_name")!="凤凰星" && !wizardp(me)) {
			write("你在天上转了半天也不知该往那边飞。。。\n");
			message_vision("\n$N失望地从云上跳了下来！\n", me);
			return 1;
		}
		loc="/d/fenghuang/zhongxin.c";
	}
	else if(arg=="wusheng"){
		loc = "/d/wusheng/shijie3.c";
	}
		...
		loc = "/d/qujing/nverguo/towna1.c";
		/*
		if (me->query("obstacle/nverguo")!="done" && !wizardp(me)){
			write("\n一阵风把你东倒西歪地吹了回来！\n");
			write("看样子飞不过去。\n");
			return 1;
			}
...
NOR, me);
		...
		message_vision(HIW"\n突然，晴空一声霹雳，天地色变，$N"+ridemsg+"如闪电般现出身来。 \n"NOR, me);
	} else if( (string)me->query("family/family_name")=="南海普陀山") {
		message_vision(HIY"\n一声洪亮的“南无阿弥陀佛”，$N"+ridemsg+"从佛光中，祥云里走出。 \n"NOR, me);
	} else if( (string)me->query("family/family_name")=="东海龙宫") {
		message_vision(HIC"\n半空中仿佛冲出一条水柱，$N"+ridemsg+"从水柱上稳稳跳落地下。  \n"NOR, me);
	} else if( (string)me->query("family/family_name")=="凤凰星") {
		message_vision(HIR"\n一团血色的烈火烧了过来,$N"+ridemsg+"身上燃着熊熊烈火，从里面缓缓走了出来. \n"NOR, me);
	} else if( (string)me->query("family/family_name")=="火云洞") {
...
wuzhuang        ->万寿山五庄观           shushan        ->蜀山

baoxiang	->宝象国　　　　pingding  	->平顶山
yalong    	->压龙山　　　　wuji      	->乌鸡国
chechi		->车迟国　　　　tongtian  	->通天河
jindou  	->金兜山　　　　nverguo   	->女儿国
...
liusha  	->流沙河　　　　baigushan 	->白骨山
huangfeng	->黄风岭　　　　lingshan  	->灵山
yingchou	->鹰愁涧　　　　heifeng   	->黑风岭
