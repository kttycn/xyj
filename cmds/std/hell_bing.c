...
//hell_bing.c...Weiqi 12/12/97.
//command to let hell people get qin bing.
...
{
	string QINBING_FILE = "/obj/npc/hellbing.c";
	object qinbing;
	int max_bing = 0;
	int current_bing = 0;
...
*/
	...
	if( (string)me->query("family/family_name") != "阎罗地府" )
    		return notify_fail("你并非地府人物，怎能领导归卒？\n");
	...
	me->set_temp("hell_current_bing", current_bing+1);

	if( (int)me-> query_temp("hell_current_bing") > max_bing )
		return notify_fail("按你的级别，领鬼兵的规格不能再高了。\n");

	//ready to get qin bing out...btw, let the player pay first.
	//1 gold each qin bing.
	if( (int)me->query("balance") < 10000)
	{
		return notify_fail("你现在已经发不出钱来了，谁还跟你？\n");
	}
...
{
    write(@HELP
指令格式: hell_bing (巫师可用hell_bing find) 
