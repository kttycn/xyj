...
int main(object me, string arg)
{
	...
	{
		case 2:
			return notify_fail(HIR"这个功夫在月影奇缘中已经存在，你没有办法再创新。\n"NOR);
			break;
		case 0: 
			break;
		case 1:
		...
		default:
			return notify_fail(HIR"请和菲飞联系，所创功夫不对。\n"NOR);
	}
	if(skill_num < 1)
		return notify_fail(HIR"你需要从第一招开始创建。\n"NOR);
	if((int)me->query("combat_exp") < (skill_num)*100000+900000)
	...
	(int)me->query_skill(spe_skill_name) < (skill_num-1)*10)
		return notify_fail(HIR"你的招式还不够熟练，先提高它们，然后在想新招吧。\n"NOR);
	...
	if(skill_num_result > 1) {
		...
		return notify_fail(HIR"你所要创建招式数可能已经成功，请与菲飞联系，或者尝试创建另外一招。\n"NOR);
...
void skill_init_header(string skill_en_name,string skill_ch_name)
{
	...
	header="// 这是玩家自创武功程序 原由maxim@nju_fengxue编写，现由swallow@xjqy修改\n";
	header+="// "+geteuid(me)+"\n";
...
void finish_write_skill(object ob)
{
	...
	action="\n([\n"+SHUANGYIN_HAO+"action"+SHUANGYIN_HAO+" :"
		+SHUANGYIN_HAO+spe_skill["action"]+SHUANGYIN_HAO+",\n"
		+SHUANGYIN_HAO+"damage"+SHUANGYIN_HAO+" :"+ spe_skill["force"]+",\n"
		+SHUANGYIN_HAO+"dodge"+SHUANGYIN_HAO+" :"+ spe_skill["dodge"]+",\n"
		+SHUANGYIN_HAO+"parry"+SHUANGYIN_HAO+" :"+ spe_skill["parry"]+",\n"
		+SHUANGYIN_HAO+"damage_type" +SHUANGYIN_HAO+": "+SHUANGYIN_HAO+spe_skill["damage_type"]
		+SHUANGYIN_HAO+",\n"+SHUANGYIN_HAO
...
越来越多，招式的威力也越来越大。每创造一种武功必须要损失20K武学，
而每增加一招需要损失8K武学。玩家增加招式的熟练程度与其它武功一致，
均可通过practice(lian)的方式提高。
