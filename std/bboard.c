...
string long()
{
	...
	for(i=sizeof(notes)-1; i>=0; i--)
		msg += sprintf("%3d %s %s %-25s %s %s\n",
			...
			notes[i]["title"]);
		msg+="[1;44m»¶Ó­¹âÁÙ" + CHINESE_MUD_NAME + 
		"                             ±¾Õ¾µØÖ·£º" + 
...
int do_post(string arg)
{
	...
	if(! wizardp(me) && (me->query_skill("literate",1))<10)
		return notify_fail("»¹ÊÇÏÈÈ¥Ñ§µã¶ÁÊéĞ´×Ö°É£¡\n");
...
int do_re(string arg)
{
	...
	if(!intp(atoi(arg)) ) return notify_fail("ÄãÖ»ÄÜRe¾ßÌåµÄÒ»ÆªÎÄÕÂ£¬ÓÃ<re Êı×Ö>±íÊ¾\n");
...
int do_read(string arg)
{
	...
	"[47;34m  ±êÌâ [44;37m %-64s[44;37m[%4d]\n"+
	"[47;34m  Ê±¼ä [44;37m %-30s  [36m        [44;33m¡Ñ From : %-20s[m\n"+
	"[36m\n------------------------------------------------------------------------------[m\n\n",
	notes[num]["author"],name(),notes[num]["title"],num+1,ctime(notes[num]["time"]),notes[num]["ip"])
	+ notes[num]["msg"] +
	"[36m\n------------------------------------------------------------------------------[m\n"+
	sprintf("[1;32m¡ù Origin : [2;37;0m[33m"+CHINESE_MUD_NAME+"[36m"+INTERMUD_MUD_NAME" [37m< "+
	MUDLIB_IP+" "+MUDLIB_PORT+" >[m\n" ));
