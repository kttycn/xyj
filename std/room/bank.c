...
{        
	...
	if (amount / 10000000000) {
		output = chinese_number(amount / 10000000000 ) + "块月影金砖";
		amount %= 10000000000;
	}
        else
        	output = "";
	if (amount / 100000000) {
		output = chinese_number(amount / 100000000 ) + "条月影金条";
		amount %= 100000000;
	}
		...
		output = "";
	if (amount / 1000000) {
		output = chinese_number(amount / 1000000 ) + "个月影金币";
		amount %= 1000000;
	}
	else
		output = "";
...
int pay_player(object who, int amount)
{
        ...
        if (amount < 1)
                amount = 0;
        if (v = amount / 10000000000) {
                ob = new("/obj/money/gold-brick");
                ob->set_amount(amount / 10000000000);
		if(!(ob->move(who)))
		    return amount;
        amount %= 10000000000;
        }
        if (v = amount / 100000000) {
                ob = new("/obj/money/gold-bar");
                ob->set_amount(amount / 100000000);
		if(!(ob->move(who)))
		    return amount;
        amount %= 100000000;
        }
