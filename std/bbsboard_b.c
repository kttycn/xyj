...
#define MAX_PLAN		3// 最多容纳 100 个贴子

#define DEL_TO		3 // 假如贴子超过了 MAX_PLAN，删掉前 50 个

#define SAVE_ALL_OLD_PLAN 0 // 是否保存所有原来的贴子，是则设为0，否则设为 1
...
// 显示未读的贴子
	...
	num--;
	msg=sprintf(
	"[ "HIW"编号："NOR"%2d ] [ "HIW"作者："NOR"%s(%s) ] [ "HIW"时间："NOR"%-18s ]\n[ "
	HIW"标题："NOR"%-54s ]\n────────────────────────────────\n%s\n"
	"──────────────────────────["HIW" 本 篇 完 "NOR"]\n",
		num + 1,
...
// 之所以 return 0 而不 return notify_fail... 是因为有 wenxuan 这个指令
	if (!arg) return 0;
	if (sscanf(arg,"%d",num)!=1) return 0;
