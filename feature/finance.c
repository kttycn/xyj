...
int can_afford(int amount)
{
        ...
        object jinbi, cash, gold, silver, coin, me=this_player();
...
{
	int total;
	object jinbi, gold, silver, coin, me=this_player();
	int jinbino, goldno, silverno, coinno;
	...
	total -= amount;
	jinbino = total/1000000;
	goldno = (total % 1000000) /10000;
	...
	if (!jinbi && jinbino>0) 
		jinbi=new("/obj/money/gold-coin");
	if (jinbi) jinbi->set_amount(jinbino);
	if (jinbino>0) jinbi->move(me);
	if (jinbino == 0 && jinbi) destruct(jinbi);
