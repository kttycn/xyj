...
void copy_npc(int sum)
{
	...
	if (t>=j) {
		...
		shout(HIW"【月影奇缘】魔界首领无天老祖大笑道：没想到如此轻易得手，真是天助我也，哈。哈。哈。下一个！\n"NOR);
...
void verdict_end()
{
	...
	if (count+1 != sizeof(factions)) {
		...
		shout(HIW"【月影奇缘】魔界首领无天老祖：他奶奶的，点子手硬，闪先！\n"NOR);
		...
		shout(HIW"【月影奇缘】魔界首领无天老祖仰天狂叫：我不服！！我不服！！！我不服！！！！\n"NOR);
		call_out("juedou",5);
	}
	LOGIN_D->set_madlock(0);
...
//			if (exp > 1000) user[i]->set("combat_exp",exp);
	...
	if (count+1 != sizeof(factions)) {
		...
		shout(HIW"【月影奇缘】魔界首领无天老祖冷哼一声：下一个！\n"NOR);
		shout(HIW"【"+HIM MUD_NAME+HIW"】敌人暂时撤退了，现在解除戒严状态！\n"NOR);
		call_out("disaster",10,count+1);
	}
	else {
