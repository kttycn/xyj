...
void create()
{
	...
	if (clonep())
		return;
	CHANNEL_D->do_channel( this_object(), "sys", "信息通告系统已经启动。\n");	
	call_out("on_time",10);
}
...
void on_time()
{
	...
	CHANNEL_D->do_channel(this_object(),"new",
		msg[j]);
	call_out("on_time",150);
}
