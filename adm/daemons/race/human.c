	...
	if( userp(ob) || undefinedp(my["max_sen"]) ) {
		if( my["age"] <= 14 ) my["max_sen"] = 10*shen;
			...
			if( my["always_young"] ){
				...
				my["max_sen"] = 26 * shen -(my["age"] - 40) * 5;
		}	
		if( my["max_mana"] > 0 ) my["max_sen"] += my["max_mana"]/4;
		if(my["max_sen"]<1) my["max_sen"]=1;//mon 1/28/98
	}
/*	if( userp(ob) || undefinedp(my["maximum_force"]) || undefinedp(my["max_force"])) {
		if( my["maximum_force"] != my["max_force"] ) {
			force = my["max_force"];
			if( mapp(skills) ) {
				s += skills["force"] / 2;
				if( mapp(skill_map) )
			...
			else my["maximum_force"] = my["max_force"];
		}
	}
	if( userp(ob) || undefinedp(my["maximum_mana"]) || undefinedp(my["max_mana"])) {
		if( my["maximum_mana"] != my["max_mana"] ) {
			mana = my["maximum_mana"];
			if( mapp(skills) ) {
				s += skills["spells"] / 2;
				if( mapp(skill_map) )
			...
			else my["max_mana"] = my["maximum_mana"];
		}
	}
*/
