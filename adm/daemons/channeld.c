...
mapping channels = ([
	...
	"gwiz": ([      "msg_speak": HIG "【网际巫师】%s：%s\n" NOR,
					...
					"filter": 1,
					"intermud_emote": 1 ]),
	"moon":  ([      "msg_speak": HIB "【月光宝盒】%s：%s\n" NOR,
					"msg_emote": HIB "【月光宝盒】%s\n" NOR,
    ...
    "yyqy": ([  "msg_speak": NOR+HIC+"【月影奇缘】"+HIW+"%s：%s\n" NOR,
            "msg_emote": HIC"【月影奇缘】"HIW"%s\n" NOR,
            "intermud": GCHANNEL,
            "channel": "yyqy",
            "filter": 1,
        ]),
    "jy": ([  "msg_speak": NOR+HIB+"【交易】"+HIW+"%s：%s\n" NOR,
            "msg_emote": HIB"【交易】"HIW"%s\n" NOR,
        ]),
	...
	"mp":([     "msg_speak": HIY "【门派】"+HIG"%s: %s\n" NOR,
				...
				"msg_color": NOR+HIY,
				"menpai_only": 1 ]),
//	"news":([ "msg_speak": HIG "【新闻播报】%s：%s\n"NOR, ]),
	"ftpd":([ "msg_speak": HIW "【移形换影】%s：%s\n"NOR, ]),
]);
...
varargs int do_channel(object me, string verb, string arg, int emote,int allow_chat)
{
	...
	if( userp(me) && !allow_chat ) {
		...
		&& !wizardp(me) && userp(me) )
			return notify_fail("你现在已经长大了，不可以再使用新手频道说话。\n");
