...
void add_(int *a1, mixed *am, int i, int code)
{
    int *tmp;
    int n = 0;
    int x;
...
int do_line (int lcnt, string sbuffer)
{
	...
	add_(GtoB, mGtoB, gb_code - GBfirst, big_code);
	add_(BtoG, mBtoG, big_code - BIGfirst, gb_code);
	total++;
    }
    return(total);
}
...
{
    if (mode==SC_GBtoBIG)
	add_(GtoB, mGtoB, code - GBfirst, cmapping);
    else if (mode==SC_BIGtoGB)
	add_(BtoG, mBtoG, code - BIGfirst, cmapping);
}
